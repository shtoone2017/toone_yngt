...
class DatePickerLongDialog: UIView {
    ...
    func show(title: String, doneButtonTitle: String = "Done", cancelButtonTitle: String = "Cancel", defaultDate: NSDate = NSDate(), datePickerMode: UIDatePickerMode = .DateAndTime, callback: DatePickerCallback) {
        ...
        //邵 追加
        self.datePicker_hm.datePickerMode = .Time
        //邵 追加
    ...
    private func createContainerView() -> UIView {
        ...
        let dialogContainer = UIView(frame: CGRectMake((screenSize.width - dialogSize.width) / 2, (screenSize.height - dialogSize.height) / 3, dialogSize.width, dialogSize.height + 150))
        ...
        self.datePicker.addTarget(self, action:Selector("datePickerValueChange:"), forControlEvents: UIControlEvents.ValueChanged)
        dialogContainer.addSubview(self.datePicker)
        
        //邵 追加
        self.datePicker_hm = UIDatePicker(frame: CGRectMake(10, 200, 0, 0))
        self.datePicker_hm.autoresizingMask = UIViewAutoresizing.FlexibleRightMargin
        self.datePicker_hm.frame.size.width = 300
        self.datePicker_hm.locale = NSLocale(localeIdentifier: "ZH_CN")
        self.datePicker_hm.addTarget(self, action:Selector("datePickerValueChange:"), forControlEvents: UIControlEvents.ValueChanged)
        dialogContainer.addSubview(self.datePicker_hm)
        //邵 追加
    ...
    func buttonTapped(sender: UIButton!) {
        if sender.tag == kDatePickerDialogDoneButtonTag {
            self.callback?(date: self.titleLabel.text!)
        }
    ...
    func getFormatDate()
    {
        let dformatter_1 = NSDateFormatter()
        dformatter_1.dateFormat = "yyyy-MM-dd"
        let date_1 = dformatter_1.stringFromDate(self.datePicker.date)
        
        let dformatter_2 = NSDateFormatter()
        dformatter_2.dateFormat = "hh:mm:ss"
        let date_2 = dformatter_2.stringFromDate(self.datePicker_hm.date)
        
        self.titleLabel.text = date_1 + " " + date_2
    }
