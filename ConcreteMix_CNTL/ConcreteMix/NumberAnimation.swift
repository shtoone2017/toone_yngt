...
class NumberAnimation:NSObject{
    ...
    init (label:UILabel,TotheValue:String) {
        ...
        self.theData =  NSNumberFormatter().numberFromString(toTheValue as NSString as String)!.integerValue
        self.label = label
        dataLength = TotheValue.characters.count
...
//        print("dataLength \(dataLength)")
    ...
    func numberTo(tUpdate:NSTimer){
        ...
        switch index {
        case 1 :
            if ((theAddData + self.primaryValue[0])<(NSNumberFormatter().numberFromString(toTheValue as NSString as String)!.integerValue)) {
                label.text = "\(theAddData + self.primaryValue[0])"
            }else {
                label.text = "\(NSNumberFormatter().numberFromString(toTheValue as NSString as String)!.integerValue)"
            }
        case 2 :
            let da = (theAddData)*setpow(left + 3) + self.primaryValue[0]*setpow(left + 3) + self.primaryValue[1]
            if ( da < (NSNumberFormatter().numberFromString(toTheValue as NSString as String)!.integerValue)){
