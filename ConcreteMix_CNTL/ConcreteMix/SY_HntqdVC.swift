...
class SY_HntqdVC: UIViewController, UITableViewDataSource, UITableViewDelegate,KSRefreshViewDelegate,GroupIDSelectDelegate,SheBeiSelectDelegate {
    ...
    func CreateTitle(){
        ...
        lb_project.text = con.departName_SY_sel + ">试验室管理>混凝土强度"
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if (cell == nil) {
            let nibs:NSArray = NSBundle.mainBundle().loadNibNamed("SY_HntqdCell", owner: self, options: nil)
            cell = nibs.lastObject as! SY_HntqdCell
            ...
            if let JSON = response.result.value {
                ...
                if(success == true){
                    ...
                    for(var i:Int=0; i<cnt; i++){
                        ...
                        self.array.append((JSON.valueForKey("data"))![i].valueForKey("SJQD")! as! String)
                        self.array.append((JSON.valueForKey("data"))![i].valueForKey("QDDBZ")! as! String)
