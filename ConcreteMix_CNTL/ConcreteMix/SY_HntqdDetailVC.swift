...
class SY_HntqdDetailVC:  UIViewController,UITableViewDataSource, UITableViewDelegate, UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,ChartViewDelegate{
    ...
    @IBOutlet weak var lb_shijianQd: UILabel!
    @IBOutlet weak var lb_qdvalue: UILabel!
    @IBOutlet weak var lb_lingqi: UILabel!
    @IBOutlet weak var shijianCc: UILabel!
    ...
    //Json Url & 参数 Begin##############################################################################################################################
    var url = con.hntkangyaDetailURL
    var SYJID = "SYJID="
    //Json Url & 参数 End################################################################################################################################
    
    //数据接收部 Begin+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    var array_KYLZ:[String] = []
    var array_KYQD:[String] = []
    //数据接收部 End+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    override func viewDidLoad() {
        ...
        lb_project.text = con.departName_SY_sel + ">试验室管理>混凝土强度详情"
        ...
        //Label
        let btn_lb02 = UILabel(frame: CGRect(x: 5, y: 290, width: con.width - 20, height: 30))
        ...
        //图标
        let img02 = UIImageView(frame: CGRect(x: con.width-40, y: 295, width:20 , height: 20))
    ...
    func getJsonData(){
        ...
        Alamofire.request(.GET, url + SYJID + id).responseJSON{ response in
            ...
            if let JSON = response.result.value {
                ...
                if(success == true){
                    //设备标号
                    //self.shebeibianhao = ((JSON.valueForKey("data"))!.valueForKey("SBBH")! as? String)!
                    ...
                    self.lb_weituoBh.text = (JSON.valueForKey("data"))!.valueForKey("WTBH")! as? String
                    self.lb_shijianBh.text = (JSON.valueForKey("data"))!.valueForKey("SJBH")! as? String
                    self.lb_shijianQd.text = (JSON.valueForKey("data"))!.valueForKey("SJQD")! as? String
                    self.lb_qdvalue.text = (JSON.valueForKey("data"))!.valueForKey("QDDBZ")! as? String
                    self.lb_lingqi.text = (JSON.valueForKey("data"))!.valueForKey("LQ")! as? String
                    self.lb_lingqi.text = self.lb_lingqi.text! + " 天"
                    self.shijianCc.text = (JSON.valueForKey("data"))!.valueForKey("SJCC")! as? String
                    ...
                    //荷载，强度
                    let str_KYLZ = (JSON.valueForKey("data"))!.valueForKey("KYLZ")! as? String
                    let str_KYQD = (JSON.valueForKey("data"))!.valueForKey("KYQD")! as? String
                    self.array_KYLZ = str_KYLZ!.componentsSeparatedByString("&")
                    self.array_KYQD = str_KYQD!.componentsSeparatedByString("&")
                    //曲线X，Y
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if (cell == nil) {
            let nibs:NSArray = NSBundle.mainBundle().loadNibNamed("SY_HntqdDetailCell", owner: self, options: nil)
            cell = nibs.lastObject as! SY_HntqdDetailCell
            
            let lb1  = cell!.viewWithTag(1) as! UILabel
            let lb2  = cell!.viewWithTag(2) as! UILabel
            let lb3  = cell!.viewWithTag(3) as! UILabel
            
            lb1.text  = array_KYLZ[indexPath.row*n+0]
            lb2.text  = array_KYQD[indexPath.row*n+0]
            lb3.layer.backgroundColor = con.blueMore.CGColor
    ...
    func createLine(){
        ...
        //放大倍数
//        let beishu = yval.count/15
//        _chartView.zoom( CGFloat(beishu), scaleY: CGFloat(beishu), x: 0, y: 0)
